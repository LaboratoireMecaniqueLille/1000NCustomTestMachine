...
if __name__ == '__main__':
  ...
  print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
        "Launching lancement_essai.py")

  BASE_DIR = Path(os.environ.get('BASE_DIR'))
  if not BASE_DIR:
      raise EnvironmentError("The variable BASE_DIR was not defined")

  # ToDo: Il ne s'est rien passé après le premier message de print ? A quoi il
  #  sert ?
  print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
        "Launching the test")
  ...
  try:
    ...
    # ToDo: Pourquoi ast ? Tu stockes une expression ? Si c'est le cas alors
    #  ce n'est pas recommandé, sinon float ou int fait l'affaire
  ...
  # ToDo: Pas de except tout seul, toujours except ValueError par example. Il
  #  faut être clair sur quelle(s) exception(s) tu catch
  except:
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
    ...
    try:
        sys.exit(1)
    # ToDo: Idem, quelle exception pourrait causer ça ?
    except Exception as e:
        tk.messagebox.showerror("Error", f"The program could not close ",
                                f"Error: {str(e)}",)
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
              f"The program could not close :\n{str(e)}")

  if test_type == "Monotonic Uniaxial Tensile Test":

    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
          "Starting the monotonic uniaxial tensile test now \n")
    ...
    crappy.link(load_cell, gen)  # besoin de savoir la force pour déterminer
    # la consigne
